...
function DashboardPagosResumen({ data, setView }) {
  ...
  useEffect(() => {
    ...
    async function cargarPagos() {
      try {
        if (!SUPABASE_URL || !SUPABASE_KEY || !data?.user?.id) return;

        const url =
          `${SUPABASE_URL}/rest/v1/pagos` +
          `?select=*&user_id=eq.${Number(data.user.id)}&order=created_at.desc`;
        ...
        if (!res.ok) {
          throw new Error(result?.message || result?.details || "No se pudieron cargar pagos.");
        }
        ...
        console.error("Error cargando pagos en dashboard:", err);
        if (activo) setError("No se pudo cargar el resumen de pagos.");
      }
    }
    ...
    <section className="dash-payments-panel">
      <div className="dash-payments-head">
        <div>
          ...
          <p>Visualiza un resumen de tus pagos de servicios.</p>
        </div>

        <button type="button" onClick={() => setView("pagos")}>
          Ir a pagos
        </button>
      </div>

      <div className="dash-payments-metrics">
        <article>
        ...
        <article>
          <span>Total pagado en servicios</span>
        ...
        <article>
          ...
          <strong>{ultimoPago ? String(ultimoPago.servicio).toUpperCase() : "Sin pagos"}</strong>
          <p>{ultimoPago ? `Contrato: ${ultimoPago.numero_contrato}` : "Aun no hay pagos"}</p>
        </article>
      </div>

      {error && <p className="dash-payments-error">{error}</p>}

      <div className="dash-payments-list">
        {pagos.length === 0 ? (
          <p className="dash-payments-empty">Aun no hay pagos registrados.</p>
        ) : (
          pagos.slice(0, 4).map((pago) => (
            <article className="dash-payment-row" key={pago.id}>
              <div>
                <strong>{String(pago.servicio).toUpperCase()}</strong>
                <span>Contrato: {pago.numero_contrato}</span>
              </div>
    ...
    <>
      ...
      <section className="smart-bank-strip">
        ...
        <button className="smart-card" onClick={() => setView("transferencias")}>
          ...
          <p>Transfiere dinero entre cuentas registradas.</p>
        </button>
      ...
      <section className="promo-grid">
        ...
        <button className="promo-card promo-dark" onClick={() => setView("transferencias")}>
          ...
          <p>Envia dinero a otra cuenta de forma rapida y segura.</p>
        </button>
      </section>

      <DashboardPagosResumen data={data} setView={setView} />
      ...
      <section className="split-grid">
        ...
        <Panel title="Ultimos movimientos de cuenta">
        ...
        <Panel title="Nueva transferencia">
          ...
          <form className="form-grid transfer-form" onSubmit={submit}>
            ...
            <input
              ...
              placeholder="Ejemplo: apoyo familiar, alquiler o envio personal"
            />
