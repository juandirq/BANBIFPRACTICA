...
def api_transfer(request: Request, data: dict = Body(...)):
    with Session(engine) as session:
        ...
        origin = session.get(Account, origin_account_id)
        destination = session.exec(select(Account).where(Account.account_number == destination_account_number)).first()

        if not origin or origin.user_id != user.id:
            return JSONResponse({"ok": False, "message": "Cuenta origen invalida"}, status_code=400)

        if not destination:
            return JSONResponse({"ok": False, "message": "La cuenta destino no existe"}, status_code=400)

        if amount <= 0 or origin.balance < amount:
            return JSONResponse({"ok": False, "message": "Monto invalido o saldo insuficiente"}, status_code=400)
        ...
        return {"ok": True, "message": "Transferencia realizada correctamente"}
